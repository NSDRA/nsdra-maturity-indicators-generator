...
public final class App {
  ...
  public static void main(final String[] args) throws IOException {
    ...
    if (!"".equals(cli.sourcePath.trim()) && !"".equals(cli.destPath.trim())) {
      ...
      int updatedCount = MiUtils.generateMaturityIndicators(cli.sourcePath,
          cli.destPath, 0);
      System.out.println(updatedCount + " maturity indicators updated");

      MiUtils.generateJsonFromMiLists(cli.sourcePath, cli.destPath
          + FileSystems.getDefault().getSeparator() + "lists.json");
    }
  }
}
